...
from maad.utils.experiment_utils import SpatioTemporalGaussianWithOpticFlowAwarenessEstimator


class AwarenessEstimationExperiment(MAADExperiment):
    """
    Awareness Estimation Experiment.

    Class for compute MSE and ABS error on awareness outputs predicted by MAAD as well as Optic Flow based SpatioTemporal Gaussian Baseline.
    """
    ...
    def initialize_functors(self):
        ...
        def awareness_output_functor(
            ...
            """
            Compute MSE and ABS error on awareness estimate from MAAD as well as Optic Flow based SpatioTemporal Gaussian Baseline
            """
            ...
            maad_awareness_estimates = []

            # init error lists
            awareness_sq_err_maad = []
            awareness_abs_err_maad = []
            ...
            for b, x, y in zip(awareness_b, awareness_x, awareness_y):
                ...
                spatio_temporal_gaussian_filter_with_of_estimator = SpatioTemporalGaussianWithOpticFlowAwarenessEstimator(
                ...
                # get estimate at coordinate using MAAD
                maad_estimate = float(awareness_map[b, y, x].cpu().float())

                # append results
                of_st_awareness_estimates.append(of_based_spatio_temporal_gaussian_baseline)
                maad_awareness_estimates.append(maad_estimate)
                target = float(awareness_target[b].cpu().float())

                # append metrics
                awareness_sq_err_maad.append((maad_estimate - target) ** 2)
                awareness_abs_err_maad.append(maad_estimate - target)
            ...
            print(
                "Sq Errors: maad: {},  OF SpatioTemporal {}".format(
                    np.average(awareness_sq_err_maad), np.average(awareness_sq_err_of_spatiotemporal_gaussian)
                )
            )
            print(
                "Abs Errors: maad: {},  OF SpatioTemporal {}".format(
                    np.average(awareness_abs_err_maad), np.average(awareness_abs_err_of_spatiotemporal_gaussian)
                )
            )

            # if any of the errors are greater than one then there is an error.
            if (
                np.max(np.abs(awareness_sq_err_maad)) > 1
                or np.max(np.abs(awareness_sq_err_of_spatiotemporal_gaussian)) > 1
                or np.max(np.abs(awareness_abs_err_maad)) > 1
            ...
            experiment_results_aggregator[AWARENESS_ERROR_MAAD_KEY].extend(awareness_sq_err_maad)
            experiment_results_aggregator[AWARENESS_ERROR_OF_SPATIOTEMPORAL_GAUSSIAN_KEY].extend(
                awareness_sq_err_of_spatiotemporal_gaussian
            )

            experiment_results_aggregator[AWARENESS_ABS_ERROR_MAAD_KEY].extend(awareness_abs_err_maad)
            ...
            experiment_results_aggregator[AWARENESS_ESTIMATE_MAAD_KEY].extend(maad_awareness_estimates)
