...
class MAADDenoisingExperiment(MAADExperiment):
    """
    ...
    def __init__(self, args, session_hash):
        ...
        self.result_keys = [
            GAZE_ERROR_MAAD_KEY_WITH_GAZE,
            GAZE_ERROR_MAAD_KEY_WITHOUT_GAZE,
    ...
    def initialize_functors(self):
        ...
        def maad_denoising_input_functor(batch_input, aux_info_list, input_process_params):
            """
        ...
        self.model_wrapper.input_process_dict["functor"] = functools.partial(maad_denoising_input_functor)
        ...
        def maad_denoising_output_functor(
            ...
            coord_err_L2_maad_meanshift_with_gaze = 0.0
            # error between ground truth and after meanshift on gaze map without gaze
            coord_err_L2_maad_meanshift_without_gaze = 0.0
            ...
            for b in range(gaze_coordinates.shape[0]):  # batch size
                ...
                for t in range(gaze_coordinates.shape[1]):  # sequence length
                    ...
                    coord_err_L2_maad_meanshift_with_gaze += with_gaze_error
                    coord_err_L2_maad_meanshift_without_gaze += without_gaze_error
            ...
            experiment_results_aggregator[GAZE_ERROR_MAAD_KEY_WITH_GAZE].append(
                coord_err_L2_maad_meanshift_with_gaze / bt_counter
            )
            experiment_results_aggregator[GAZE_ERROR_MAAD_KEY_WITHOUT_GAZE].append(
                coord_err_L2_maad_meanshift_without_gaze / bt_counter
            )
        ...
        self.model_wrapper.output_process_dict["functor"] = functools.partial(maad_denoising_output_functor)
...
def main():
    ...
    for noise_level in args.experiment_noise_levels:
        ...
        denoising_experiment = MAADDenoisingExperiment(args_copy, session_hash)
