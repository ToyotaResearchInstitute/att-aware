...
from maad.model.MAADNet import MAADNet
...
def load_datasets(params_dict):
    ...
    if not params_dict["use_std_train_test_split"]:
        ...
        awareness_datasets["test"] = MAADAttAwarenessDataset("test", params_dict)
        print("Creating Gaze Test DS")
        gaze_datasets["test"] = MAADGazeDataset("test", params_dict)
    else:
        """
    ...
    return (
        ...
        (gaze_dataset_indices, awareness_dataset_indices, pairwise_gaze_dataset_indices,),
    )
...
def generate_train_test_split_indices(dataset, params_dict):
    ...
    Parameters
    ----------
    dataset: Pytorch Dataset (MAADGazeDataset, MAADAttAwarenessDataset, MAADPairwiseGazeDataset)
...
def create_model_and_loss_fn(params_dict):
    ...
    loss_fn = MAADLoss(params_dict, gt_prior_loss=partial(gaze_transform_prior_loss))
...
def process_and_extract_data_batch(
    ...
    sample_to_device(
        (gaze_data_batch, awareness_data_batch, pairwise_gaze_data_batch_t, pairwise_gaze_data_batch_tp1,), device,
    )
...
def extract_individual_batch_input(
    ...
    # extract maad input from each batch
        ...
        if "post_parse_data_item" in input_process_dict:
            if "functor" in input_process_dict["post_parse_data_item"]:
                ...
                gaze_batch_input, gaze_aux_info_list, gaze_batch_should_use_batch = post_parse_data_item_functor(
                    gaze_batch_input, gaze_aux_info_list, input_process_dict["post_parse_data_item"]["params"],
                )
