...
class MAADInferenceEngine(object):
    """
    Class that defines the inference engine using the MAAD.
    """
    ...
    def infer(self, module):
        """
        Performs inference using MAAD model.
        Parameters:
        ----------
        module: ModelWrapper
        ...
        if self.inference_ds_type == "train":
            dataloader_tqdm = tqdm.tqdm(
                enumerate(zip(gaze_dataloaders["train"], awareness_dataloaders["train"])), desc="inference_train",
            )
        elif self.inference_ds_type == "test":
            dataloader_tqdm = tqdm.tqdm(
                enumerate(zip(gaze_dataloaders["test"], awareness_dataloaders["test"])), desc="inference_test",
            )
