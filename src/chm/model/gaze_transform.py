...
def compute_inverted_affine_transform(linear_mtx, trans_vec):
    ...
    Parameters:
    -----------
    linear_mtx: np.matrix
        The A matrix of an affine transform of the form y = Ax + b
    trans_vec: np.array
        The b vector of an affine transform of the form y = Ax + b

    Returns:
    --------
    inverted_affine_transform: tuple
        tuple containing A matrix and b vector of the inverse transform. This is computed as
        for  y = Ax + b ---> x = A^-1( y - b) --> A' = A^-1 and b' = -(A^-1)*b
    """
    result_mtx = np.linalg.pinv(linear_mtx)
    result_vec = -np.dot(result_mtx, trans_vec)
    inverted_affine_transform = (result_mtx, result_vec)
    return inverted_affine_transform
